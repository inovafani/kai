...
function buildInput(input: Parameters<AssistantLlmClient["composeReply"]>[0]) {
  const history = (input.conversationHistory ?? [])
    ...
    .join("\n");
  const conciergeMode = input.requiredFacts.length === 0;

  const instructions = [
    conciergeMode
      ? "Answer the traveller naturally and helpfully using the deterministic reply as grounding, not as a script."
